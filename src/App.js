...
function App() {
    ...
    const getvalidate = (e)=>{
      ...
      if(!!value.match(/^[0-9]*.?[0-9]+$/)){   //to convert boolean 
        if(name==='principle'){
      setPrinciple(value)
      setIsPrinciple(true)
    }
  ...
  return (
    <div style={{height:'100vh'}} className='d-flex justify-content-center align-items-center w-100 bg-dark'>
     ...
     <form className='mt-5' onSubmit={handleCalculate}>
      ...
      <div className='mb-3'>
         <TextField name='rate' value={rate ||""}  className='w-100' id="outlined-basic" label="Rate Of Interest (p.a) %" variant="outlined"
         onChange={(e)=>getvalidate(e)} />
      </div>
      {!isRate &&
      <div>
      ...
      <div className='mb-3'>
         <TextField name='year' value={year || ""}  onChange={(e)=>getvalidate(e)} className='w-100' id="outlined-basic" label="Year (Yy)" variant="outlined" />
      </div>
      {!isYear &&
      <div>
        <p className='text-danger'>Invalid Input</p>
      ...
      <Button type='submit' disabled={isPrinciple && isRate && isYear?false:true} className='bg-success' 
      style={{width:'200px', height:'50px'}} variant="contained">Calculate</Button>
